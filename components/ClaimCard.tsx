...
import { supabase, type Claim, type EventLog, type PaymentConfirmation } from '@/lib/supabase'
...
export default function ClaimCard({ claim: c, onEdit, onDelete, onStatusChange }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<'details' | 'events' | 'payments'>('details')
  ...
  const addEvent = async (type: EventLog['type'], label: string, detail?: string) => {
    ...
    const updates: Partial<Claim> = { events_log: JSON.stringify(updatedEvents) }
  ...
  const handleDepart = async () => {
    if (!confirm('Mark this vehicle as departed / picked up? Status will be set to Complete.')) return
    setSavingEvent(true)
  ...
  const handleAddPayment = async () => {
    ...
    // Also update paid amount on the claim and log event
    ...
    <div className={`card transition-all ${expanded ? 'ring-1 ring-gray-300' : ''}`}>
      ...
      <button className="w-full text-left p-4 sm:p-5" onClick={() => setExpanded(!expanded)}>
        ...
        <div className="grid grid-cols-4 gap-2 mt-3 pt-3 border-t border-gray-100">
          ...
          <div><p className="text-xs text-gray-400">Estimate</p><p className="text-xs font-medium text-gray-700 mt-0.5">{formatMoney(c.est_amount)}</p></div>
      ...
      {expanded && (
        <div className="border-t border-gray-100">
          ...
          {/* Tabs */}
          <div className="flex border-b border-gray-100 mx-4 sm:mx-5 mt-4">
            {(['details', 'payments', 'events'] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`px-4 py-2 text-xs font-medium border-b-2 transition-colors capitalize ${activeTab === tab ? 'border-gray-900 text-gray-900' : 'border-transparent text-gray-400 hover:text-gray-600'}`}>
                {tab === 'payments' ? `Payments (${payments.length})` : tab === 'events' ? `Events (${events.length})` : 'Details'}
              </button>
            ))}
          </div>
            ...
            <div className="px-4 sm:px-5 pb-5 pt-4">
              ...
              {c.file_name && (
                <div className="mt-3">
                  ...
                  {c.file_url
                    ? <a href={c.file_url} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="inline-flex items-center gap-2 px-3 py-2 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-lg text-xs text-blue-600 transition-colors">
                        <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                        {c.file_name}
                ...
                <div className="border border-gray-200 rounded-xl p-4 space-y-3 bg-gray-50">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input className="form-input" placeholder="$0.00" value={payAmount} onChange={e => setPayAmount(e.target.value)}/>
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <input className="form-input" placeholder="e.g. Final payment, partial, deductible collected..." value={payNote} onChange={e => setPayNote(e.target.value)}/>
                  </div>
                  {/* Screenshot upload */}
                  <div>
